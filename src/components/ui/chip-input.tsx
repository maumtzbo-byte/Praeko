...
export function ChipInput({ value, onChange, placeholder, className }: ChipInputProps) {
  ...
  return (
    <div
      className={cn(
        "flex min-h-11 w-full flex-wrap items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-2.5 py-1.5 focus-within:border-zinc-400 focus-within:ring-4 focus-within:ring-zinc-100 ",
        className,
      )}
    >
      {value.map((chip) => (
      ...
      <input
        ...
        className="min-w-[120px] flex-1 border-none bg-transparent px-1 py-1 text-sm text-zinc-900 outline-none placeholder:text-zinc-400 "
      />
    </div>
  );
}
